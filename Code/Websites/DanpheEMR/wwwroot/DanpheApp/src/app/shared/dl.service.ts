...
export class DLService {
  ...
  public TotalItemsBillReportAdvanceFilter(schemeId: number, billingType: string, visitType: string, performerId: number, departmentId: number, serviceDepartmentId: number, serviceItemId: number, prescriberId: number, employeeId: number, fromDate: string, toDate: string) {
    return this.http.get<any>(`/BillingReports/TotalItemsBillAdvanceFilter?FromDate=${fromDate}&ToDate=${toDate}&SchemeId=${schemeId}&BillingType=${billingType}&VisitType=${visitType}&PerformerId=${performerId}&departmentId=${departmentId}&ServiceDepartmentId=${serviceDepartmentId}&ServiceItemId=${serviceItemId}&PrescriberId=${prescriberId}&EmployeeId=${employeeId}`, this.options);
  }
  GetBillingServiceItemList() {
    return this.http.get<any>("/api/BillSettings/BillingItemList", this.options);
  }
  ...
  public GetAgeGroups() {
    return this.http.get<DanpheHTTPResponse>("/api/Master/GetAgeGroups", this.options)
  }
}
