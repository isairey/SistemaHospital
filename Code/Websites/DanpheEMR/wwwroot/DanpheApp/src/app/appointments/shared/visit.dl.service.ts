...
export class VisitDLService {
  ...
  public GetSSFSubProduct(claimCode: number) {
    return this.http.get<any>(`/api/SSF/GetSSFSubProduct?claimCode=${claimCode}`, this.jsonOptions);
  }
  ...
  public GetLatestClaimCodeForAutoGeneratedClaimCodes(creditOrganizationId: number) {
    return this.http.get<DanpheHTTPResponse>(`/api/Visit/GetLatestClaimCode?creditOrganizationId=${creditOrganizationId}`);
  }
  public GetPatientLastVisitDetail(patientId: number) {
    return this.http.get<DanpheHTTPResponse>(`/api/Visit/GetPatientPreviousVisit?patientId=${patientId}`);
  }
  public GetPatientVisitsDetails(patientId: number) {
    return this.http.get<DanpheHTTPResponse>(`/api/Visit/PatientVisits?patientId=${patientId}`);
  }
  public GetPatientVisitsBySearchFilterOption(filterBy: string, searchTxt: string) {
    return this.http.get<any>(`/api/Visit/SearchPatientVisits?filterBy=${filterBy}&searchText=${searchTxt}`);
  }
}
