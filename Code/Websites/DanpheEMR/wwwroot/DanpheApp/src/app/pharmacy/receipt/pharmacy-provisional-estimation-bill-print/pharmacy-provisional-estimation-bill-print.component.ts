...
import { PharmacyProvisionalEstimationBillReceipt_DTO } from "./pharmacy-provisional-estimation-bill-receipt.dto";

@Component({
    selector: 'pharmacy-provisional-estimation-bill-print',
    templateUrl: 'pharmacy-provisional-estimation-bill-print.component.html'
})
export class PharmacyProvisionalEstimationBillPrintComponent {

    @Input('provisional-estimation-invoice') ProvisionalEstimationBillInvoice: PharmacyProvisionalEstimationBillReceipt_DTO = new PharmacyProvisionalEstimationBillReceipt_DTO();
    ...
    ngOnInit() {
        console.log(this.ProvisionalEstimationBillInvoice);
        ...
        this.UserName = this.securityService.loggedInUser.Employee.FullName;
    }
    ...
    GetPharmacyItemNameDisplaySettings() {
        ...
        if (checkGeneric != null) {
            let pharmacyItemNameSettingValue = JSON.parse(checkGeneric.ParameterValue);
            this.showGenericName = pharmacyItemNameSettingValue.Show_GenericName
            this.showItemName = pharmacyItemNameSettingValue.Show_ItemName;
            this.showGenNameAfterItemName = pharmacyItemNameSettingValue.Show_GenericName_After_ItemName;
            this.LeadingSeparator = pharmacyItemNameSettingValue.Separator.trim();
    ...
    * @param separator string that separates item name and generic name, wild card value "brackets" uses '()' to separate item name and generic name
    * @param showGenericNameAfterItemName true if item name should be seen first and generic name should be seen after
    * @returns void
    ...
    */
    public UpdateItemDisplayName(showGenericName: boolean, showItemName: boolean, separator: string = '', showGenericNameAfterItemName: boolean) {
        for (var i = 0; i < this.ProvisionalEstimationBillInvoice.ProvisionalEstimationBillInvoiceItems.length; i++) {
            var item = this.ProvisionalEstimationBillInvoice.ProvisionalEstimationBillInvoiceItems[i];
    ...
    public Print(idToBePrinted: string = 'provisional-estimation-invoice-print') {
