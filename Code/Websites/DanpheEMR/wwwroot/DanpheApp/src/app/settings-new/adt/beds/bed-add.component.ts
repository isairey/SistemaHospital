...
export class BedAddComponent {
      ...
      .subscribe(res => {
        if (res.Status == 'OK') {
          if (res.Results.length) {
            this.wardList = res.Results.filter(ward => ward.IsActive);
  ...
  public GetLastBedNumber(wardId) {
    ...
    if (filteredList.length) {
      ...
      let lastBed = filteredList.reduce(function (l, e) {
        return parseInt(e.BedNumber) > parseInt(l.BedNumber) ? e : l;
      });
  ...
  public GetBedFeatureList() {
    this.settingsBLService.GetBedFeatureList()
      .subscribe(res => {
        if (res.Status == 'OK') {
          if (res.Results.length) {
            this.bedFeatureList = res.Results.filter(bedFeature => bedFeature.IsActive);
            if (this.selectedItem)
  ...
  Add() {
    ...
    for (let i in this.CurrentBed.BedMainValidator.controls) {
  ...
  CheckBedNumber() {
    ...
    let filteredList = this.bedList.filter(a => a.WardId == this.CurrentBed.WardId);
    if (filteredList) {
      let bed = filteredList.find(a => a.BedNumber == this.CurrentBed.BedNumber);
      if (bed)
        return true;
      else
        return false;
