...
export class CoreDLService {
  ...
  public GetSalutationList() {
    return this.http.get<any>("/api/Settings/Salutation", this.options);
  }
  ...
  public GetInsuranceMasterItems() {
    return this.http.get<DanpheHTTPResponse>("/api/BillingMaster/InsuranceMasterItems", this.options);
  }
  public GetCurrentFiscalYear() {
    return this.http.get<any>("/api/Master/CurrentFiscalYear", this.options);
  }
  ...
  GetCappingResponse(NSHINumber: string) {
    return this.http.get<any>(`/api/HIB/CappingInfo?NSHINumber=${NSHINumber}`, this.options);
  }
