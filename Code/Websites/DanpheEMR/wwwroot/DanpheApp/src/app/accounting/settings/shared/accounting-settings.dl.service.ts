...
export class AccountingSettingsDLService {
  ...
  public AddSubLedger(subLedger: Array<SubLedgerModel>) {
    return this.http.post<DanpheHTTPResponse>(`/api/AccountingSettings/AddSubLedger`, subLedger, this.jsonOptions);
  }
  ...
  GetInvSubStoreList() {
    try {
      return this.http.get<any>("/api/Accounting/InventorySubStoreLedgers", this.options);
    } catch (ex) {
      throw ex
    }
  }
  ...
  public GetHospitalsList() {
    return this.http.get("/api/Accounting/Hospitals");
  }
}
