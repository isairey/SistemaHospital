...
import { ENUM_Data_Type, ENUM_DateTimeFormat, ENUM_MessageBox_Status } from "../../../shared/shared-enums";
...
export class DayBookReportComponent {
  ...
  public AllLedgerList: Array<LedgerModel> = new Array<LedgerModel>();
  ...
  LoadData() {
    try {
      ...
      this.AllLedgerList = this.accountingService.AllLedgers;
  ...
  CheckAndAssignHospital() {
    ...
    if (this.SelectedHospital) {
      this.LedgerList = this.AllLedgerList.filter(l => l.HospitalId == this.SelectedHospital);
    }
  }
  ...
  HandleLedgerChange() {
    if (this.selectedLedgerList && typeof (this.selectedLedgerList) === ENUM_Data_Type.Object)
  ...
  GetTxnList() {
    ...
    if (this.CheckSelLedger() && this.checkDateValidation()) {
      this.accReportBLService.GetDayBookReport(this.fromDate, this.toDate, this.fiscalYearId, this.selectedLedgerList.LedgerId, this.HospitalId)
        .subscribe(res => {
          if (res.Status == 'OK' && res.Results) {
            ...
            this.msgBoxServ.showMessage(ENUM_MessageBox_Status.Notice, ["No data avalilable on selected date range."]);
          }
        },
          err => {
  ...
  CheckSelLedger(): boolean {
    if (!this.selectedLedgerList || this.selectedLedgerList.length < 1) {
      this.msgBoxServ.showMessage(ENUM_MessageBox_Status.Warning, ["Select ledger from the list."]);
      return false;
    }
