...
@Component({
    selector: 'inventory-substore-ledger-mapping',
    templateUrl: "./inv-substore-ledger-mapping.component.html"
})

export class InventorySubStoreLedgerMappingComponent {
    ...
    public ShowInventorySubStoreAllLedgers: boolean = false;
    public InventorySubStoreLedgerList: Array<LedgerModel> = new Array<LedgerModel>();
    ...
    public InventorySubStoreLedgerParam = {
    ...
    public Getledgers() {
        try {
            ...
            if (ledgers.length > 0) {
                ...
                this.InventorySubStoreLedgerParam = this.LedgerTypeParamter.find(a => a.LedgerType === ENUM_ACC_ADDLedgerLedgerType.InventoryConsumption);
            } else {
        ...
        try {
            if (searchKey && searchKey.trim()) {
                ...
                if (this.ConsultantfilterType === 'withacchead') {
                    this.InventorySubList = this.InventorySubStoreLedgerList.filter(l => l.StoreName.toLowerCase().indexOf(searchKey.toLowerCase()) > -1 && l.LedgerId > 0);
                }
                else if (this.ConsultantfilterType === 'withoutacchead') {
                    this.InventorySubList = this.InventorySubStoreLedgerList.filter(l => l.StoreName.toLowerCase().indexOf(searchKey.toLowerCase()) > -1 && l.LedgerId === 0);
                }
                else {
                    this.InventorySubList = this.InventorySubStoreLedgerList.filter(l => l.StoreName.toLowerCase().indexOf(searchKey.toLowerCase()) > -1);
                }
            }
            else {
                if (this.ConsultantfilterType === 'withacchead') {
                    this.InventorySubList = this.InventorySubStoreLedgerList.filter(l => l.LedgerId > 0);
                }
                else if (this.ConsultantfilterType === 'withoutacchead') {
                    this.InventorySubList = this.InventorySubStoreLedgerList.filter(l => l.LedgerId == 0);
                }
                else {
                    this.InventorySubList = this.InventorySubStoreLedgerList;
                }
            }
        }
        catch (ex) {
    ...
    SelectAllChkOnChange() {
        if (this.IsSelectAll) {
            let ledgerObj = this.LedgerListAutoComplete.find(a => a.Name === this.InventorySubStoreLedgerParam.LedgerName)
    ...
    GetInventorySubStoreList() {
        this.accountingSettingsBLService.GetInvSubStoreList()
            .subscribe(res => {
                if (res.Status === ENUM_DanpheHTTPResponseText.OK) {
                    this.InventorySubStoreLedgerList = new Array<LedgerModel>();
                    ...
                    data.forEach((emp, index) => {
                        ...
                        this.InventorySubStoreLedgerList.push(led);
                    ...
                    if (this.InventorySubList.length > 0) {
                        this.ShowInventorySubStoreAllLedgers = true;
                    }
    ...
    ToggleInventorySubcategory(mapped) {
        if (mapped === 'true') {
            ...
            this.InventorySubList = this.InventorySubStoreLedgerList.filter(emp => emp.LedgerId > 0);
