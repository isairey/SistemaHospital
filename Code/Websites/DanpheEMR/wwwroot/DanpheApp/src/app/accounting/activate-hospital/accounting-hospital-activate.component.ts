...
export class ActivateAccountingHospitalComponent {
    ...
    constructor(private _router: Router, public accBLService: AccountingBLService, public router: Router,
            ...
            if (res.Status === ENUM_DanpheHTTPResponses.OK) {
                ...
                if (allHospitalsMst && allHospitalsMst.length) {
                    ...
                    if (this.allHospitalsPermissions && this.allHospitalsPermissions.length > 0) {
                        ...
                        this.msgBoxServ.showMessage(ENUM_MessageBox_Status.Warning, ["You don't have permission to accounting."]);
                        if (this.securityServ.HasPermission("accounting-tenant-add-view")) {
                            this._router.navigate(['/Accounting/AddTenant']);
                        }
                        else {
                    ...
                    this.msgBoxServ.showMessage(ENUM_MessageBox_Status.Warning, ["Accounting is not Configured for Hospital."]);
                    if (this.securityServ.HasPermission("accounting-tenant-add-view")) {
                        this._router.navigate(['/Accounting/AddTenant']);
                    }
                    else {
                ...
                .subscribe(async (res: DanpheHTTPResponse) => {
                    if (res.Status === ENUM_DanpheHTTPResponses.OK) {
                        ...
                        if (this.changeRequest || this.totalNumOfPermissions > 1) {
                            this.msgBoxServ.showMessage(ENUM_MessageBox_Status.Success, ['Your new Accounting Tenant is Selected']);
                        }
                        ...
                        this.msgBoxServ.showMessage(ENUM_MessageBox_Status.Error, ['Cannot activate any tenant. Please try again Later']);
                    }
                }, err => {
                    console.log(err);
                    this.securityServ.SetAccHospitalInfo(null);
    ...
    public CloseHospitalSelectionPopup() {
        this.isSinglePermissionOnly = false;
        this.securityServ.AccHospitalInfo.ActiveHospitalId = 0;
        this.router.navigate(['']);
    }
}
